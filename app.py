...
def predict_next_word():
    ...
    token_list = token_list[-31:] if len(token_list) > 31 else token_list
    token_text = []
    
    for word in token_list:
        try:
            token_text.append(tokenizer.word_index[word])
        except KeyError:
    ...
    padded_token_text = padded_token_text.reshape(padded_token_text.shape[0], 1, padded_token_text.shape[1])
    
    # Get predictions
    predictions = model.predict(padded_token_text)[0]
    ...
    return jsonify({'suggestions': top_words})
